...
class ContextRelevanceAnalyzer:
    ...
    def __init__(
    ...
    ):  # ADD codebase_context to __init__
        ...
        # --- FIX START ---
        self.last_relevant_files: List[
            Tuple[str, float]
        ] = []  # Store the last computed relevant files
        # --- FIX END ---
    ...
    def extract_relevant_code_segments(
        self, content: str, max_chars: int = 5000
    ) -> str:
        """Preserves structural elements while respecting token limits"""
        structural_elements = re.findall(
            r"^(import|from|class|def) .+", content, re.MULTILINE
        )
    ...
    def compute_file_embeddings(self, codebase_context: Dict[str, str]):
        ...
        self.codebase_context = (
            codebase_context  # Ensure internal codebase_context is updated
        )
        for file_path, content in codebase_context.items():
            ...
            try:
                embedding = self.model.encode([representation], convert_to_numpy=True)[
                    0
                ]
    ...
    def find_relevant_files(
        ...
        active_personas: Optional[Tuple[str, ...]] = None,
        ...
        for file_path, embedding in self.file_embeddings.items():
            ...
            weighted_similarity = self._apply_keyword_boost(
                file_path, base_similarity, key_terms, active_personas
            )
            # --- END MODIFICATION ---
    ...
    def _calculate_negation_impact(
        self, prompt_lower: str, keyword_start_pos: int, max_proximity: int = 75
    ) -> float:
        """
        ...
        """
        min_multiplier = (
            0.1  # Max penalty (e.g., "not keyword" makes it 10% as relevant)
        )
            ...
            for neg_match in re.finditer(neg_pattern, prompt_lower):
                ...
                if negation_end_pos < keyword_start_pos:
                    ...
                    if distance <= max_proximity:
                        closest_negation_distance = min(
                            closest_negation_distance, distance
                        )
    ...
    def _apply_keyword_boost(
        ...
        prompt_lower = " ".join(
            key_terms
        ).lower()  # Reconstruct prompt for negation search
        ...
        for term in key_terms:
            ...
            if re.search(r"\b" + re.escape(term) + r"\b", file_path_lower):
                ...
                if keyword_start_pos_in_prompt != -1:
                    negation_multiplier = self._calculate_negation_impact(
                        prompt_lower, keyword_start_pos_in_prompt
                    )
                    boost += (
                        0.1 * negation_multiplier
                    )  # Apply boost, modified by negation impact
        ...
        if active_personas:
            ...
            if "Security_Auditor" in active_personas and (
                "security" in file_path_lower
                or "auth" in file_path_lower
                or "crypto" in file_path_lower
            ):
                persona_focus_boost += 0.15
            if "Code_Architect" in active_personas and (
                "model" in file_path_lower
                or "schema" in file_path_lower
                or "dto" in file_path_lower
                or "entity" in file_path_lower
            ):
    ...
    def generate_context_summary(
        self, relevant_file_paths: List[str], max_tokens: int, debate_topic: str
    ) -> str:
        """
        ...
        for file_path in relevant_file_paths:
            ...
            for line in lines:
                ...
                if any(
                    kw in line_lower for kw in topic_keywords
    ...
    def _extract_structure_info(self, relevant_file_paths: List[str]) -> str:
        ...
        for file_path in relevant_file_paths[
            :5
        ]:  # Only look at top 5 files for high-level structure
            content = self.codebase_context.get(file_path, "")
            if not content:
                continue

...
def update_user_profile(user_id: int, profile_data: Dict[str, Any]):
    ...
    try:
        ...
        if profile_exists:
            ...
            cursor.execute("INSERT INTO user_profiles (user_id, display_name, bio) VALUES (?, ?, ?)",
...
def get_user_data_secure(username: str):
    ...
    try:
        ...
        logger.error(f"Error retrieving user data for '{username}' with parameterized query: {e}")
        return None
    finally:
        conn.close()
...
if __name__ == "__main__":
    ...
    if user_id:
        # Update profile
        update_user_profile(user_id, {"display_name": "Test User", "bio": "A test user profile."})

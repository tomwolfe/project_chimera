...
from pydantic import BaseModel, Field # Assuming BaseModel and Field are used for findings
...
class SelfImprovementFinding(BaseModel):
    ...
    metrics: Optional[QuantitativeImpactMetrics] = None # Added for quantitative metrics
...
class SelfImprovementAnalyzer:
    ...
    def analyze_codebase(self, codebase_context: Dict) -> SelfImprovementAnalysisOutput:
        ...
        findings = self._prioritize_findings(findings) # Then prioritize based on metrics
    ...
    def _analyze_reasoning_quality(self, codebase_context: Dict) -> List[SelfImprovementFinding]:
        ...
        if "src/socratic_debate.py" in codebase_context.get("files", {}):
            findings.append(SelfImprovementFinding(
    ...
    def _analyze_token_efficiency(self, codebase_context: Dict) -> List[SelfImprovementFinding]:
    ...
    def _analyze_robustness(self, codebase_context: Dict) -> List[SelfImprovementFinding]:
    ...
    def _prioritize_findings(self, findings: List[SelfImprovementFinding]) -> List[SelfImprovementFinding]:
        """
        ...
        for finding in findings:
            if finding.metrics and finding.metrics.estimated_effort and finding.metrics.estimated_effort > 0:
                ...
                finding.pareto_score = finding.priority_score # Fallback to original priority score
    ...
    def _add_quantitative_impact_metrics(self, findings: List[SelfImprovementFinding]) -> List[SelfImprovementFinding]:
        """
        ...
        for finding in findings:
            if finding.metrics is None:
                ...
                finding.metrics = QuantitativeImpactMetrics(
                    estimated_effort=finding.priority_score * 10, # Heuristic: higher priority = lower effort
                    expected_quality_improvement=finding.priority_score * 0.5, # Heuristic
    ...
    def _analyze_code_quality(self, codebase_context: Dict) -> List[SelfImprovementFinding]:
        ...
        test_coverage_summary = self.metrics.get("maintainability", {}).get("test_coverage_summary", {})

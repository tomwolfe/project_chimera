...
class ModelRegistry:
    def __init__(self):
        ...
        self.register_model(ModelSpecification(
    ...
    def get_model(self, requirements: list[str] = None, budget: float = None, preferred_model_name: Optional[str] = None) -> Optional[ModelSpecification]:
        """Get the best available model meeting requirements within budget, or a preferred model."""
        if preferred_model_name and preferred_model_name in self.models and self.models[preferred_model_name].is_available:
        ...
        for model in self.models.values():
            ...
            if requirements and not all(req in model.capabilities for req in requirements):
                continue
            ...
            if budget and (model.cost_per_1k_input > budget or model.cost_per_1k_output > budget):
                continue

...
class ConfigPersistence:
    ...
    def __init__(self, config_dir: str = "config"):
        ...
        self.custom_frameworks_dir = (
            self.config_dir / CUSTOM_FRAMEWORKS_PATH
        )  # Use the Path object relative to config_dir

        # Ensure config directory and custom frameworks directory exist
        self.config_dir.mkdir(exist_ok=True)
        self.custom_frameworks_dir.mkdir(
            parents=True, exist_ok=True
        )  # Use parents=True for nested creation
                ...
                if filename.endswith(".json"):
                    ...
                    try:
                        with open(filepath, encoding="utf-8") as f:
                            ...
                            actual_name = (
                                config_data.get("framework_name")
                        ...
                        logger.warning(
                            f"Could not read or parse custom framework file {filename}: {e}"
                        )
            return sorted(list(set(framework_names)))  # Ensure uniqueness and sort
        except OSError as e:
            logger.error(
                f"Error listing custom frameworks in {self.custom_frameworks_dir}: {e}"
            )
            return []
    ...
    def _load_custom_framework_config_from_file(
        ...
        try:
            ...
            logger.error(
                f"Error loading custom framework '{framework_name}' from '{filepath}': {e}"
            )
            return None
    ...
    def save_user_framework(
        ...
        try:
            ...
            return (
                True,
                f"Framework '{framework_name}' saved successfully to '{filepath}'!",
            )
        except OSError as e:
            logger.error(
                f"Error saving framework '{framework_name}' to '{filepath}': {e}"
            )
            return (
                False,
                f"Error saving framework '{framework_name}' to '{filepath}': {e}",
            ...
            logger.exception(
                f"An unexpected error occurred while saving framework '{framework_name}': {e}"
            )
            return (
                False,
                f"An unexpected error occurred while saving framework '{framework_name}': {e}",

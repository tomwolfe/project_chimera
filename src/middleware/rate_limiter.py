...
class RateLimiter:
    ...
    def check_and_record_call(
        ...
        self.timestamps[key] = [
            ts for ts in self.timestamps[key] if now - ts < self.period
        ]
        ...
        if not dry_run:
            ...
            self.logger.debug(
                f"Call recorded for key '{key}'. Count: {len(self.timestamps[key])}/{self.calls} in {self.period}s period."
            )
            current_count += 1  # Increment for the newly recorded call
            usage_percent = (
                current_count / self.calls
            ) * 100  # Recalculate usage percent
    ...
    def __call__(self, func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            is_allowed, count, wait_time, usage_percent = self.check_and_record_call(
                key, dry_run=False
            )  # Not a dry run for actual calls

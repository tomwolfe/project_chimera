...
class SocraticDebate:
    ...
    def _log_with_context(self, level: str, message: str, **kwargs):
        """Helper to add request context to all logs"""
        ...
        for k, v in log_data.items():
            try:
                ...
                log_data[k] = str(v) # Convert to string if not serializable
        ...
        else: # Default to error if level is unknown
    ...
    def _calculate_token_budgets(self):
        ...
        try:
            ...
            # NOTE: The original code used `self.prompt` which is not defined in __init__.
            # Assuming it should be `self.initial_prompt`.
            ...
            # Allocate remaining tokens to debate and synthesis phases
            debate_ratio = 0.9  # 90% to debate phase
            synthesis_ratio = 0.1  # 10% to synthesis phase
            
            # Minimum token allocation to ensure phases can function
            MIN_PHASE_TOKENS = 250
            
            # Calculate debate tokens with minimum guarantee
            ...
            # Synthesis gets whatever's left, but at least minimum
            ...
            # Additional safety check for extremely constrained scenarios
            if self.phase_budgets["debate"] < 100 or self.phase_budgets["synthesis"] < 100:
                ...
                # Fall back to minimum viable allocation
                total_needed = 200  # 100 for debate, 100 for synthesis
                if self.max_total_tokens_budget >= total_needed:
                    ...
                    # Extremely constrained scenario - allocate proportionally
                    self.phase_budgets = {
                        "context": 0,
            ...
            # Provide fallback budgets if calculation fails
    ...
    def count_tokens(self, text: str) -> int:
        """Counts tokens in text using the provider's tokenizer, with an enhanced fallback."""
        try:
            # Attempt to use the provider's tokenizer
            return self.llm_provider.count_tokens(text)
        except Exception as e:
            self._log_with_context("warning", "Token counting failed",
                                  method="count_tokens",
                                  error=str(e))
            # ENHANCED FALLBACK: Use word-based approximation with language-aware weighting
            # Average English words are ~1.3 tokens, code is denser (~0.8 tokens/word)
            # This handles mixed content better than pure character counting
            words = len(re.findall(r'\b\w+\b', text))
            code_heuristic = 0.8 if any(c in text for c in ['{', '}', '(', ')', '=', '#', '"""']) else 1.3
            approx_tokens = max(10, int(words * code_heuristic))
            
            # Safety clamp to prevent extreme over/under-estimation
            char_based = max(1, int(len(text) / 3))
            approx_tokens = min(approx_tokens * 1.5, char_based * 1.2)
            
            self._log_with_context("warning", "Falling back to enhanced token approximation",
                                  fallback_value=int(approx_tokens),
                                  word_based=int(words * code_heuristic),
    ...
    def track_token_usage(self, phase: str, tokens: int):
        ...
        if phase in self.tokens_used_per_phase:
            ...
            self._log_with_context("warning", f"Attempted to track tokens for unknown phase: {phase}", method="track_token_usage")
    ...
    def check_budget(self, phase: str, tokens_needed: int, step_name: str):
        ...
        if phase not in self.phase_budgets:
            self._log_with_context("warning", f"Phase '{phase}' not found in budget configuration.", method="check_budget")
    ...
    def _execute_debate_persona_turns(self, persona_sequence: List[str], context_persona_turn_results: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        # Exclude the last persona if it's a designated synthesizer for the debate loop
        ...
        if persona_sequence and persona_sequence[-1] in ["Impartial_Arbitrator", "General_Synthesizer"]:
            ...
            debate_personas_to_run = persona_sequence[:-1]
        ...
        if synthesis_persona_name and synthesis_persona_config:
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    else: # No JSON validation required
    ...
    def _execute_llm_turn(self, persona_name: str, persona_config: PersonaConfig, prompt: str, phase: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.check_budget(phase, 0, f"End of {persona_name} turn")
    ...
    def get_progress_pct(self, phase: str, completed: bool = False, error: bool = False) -> float:
        ...
        if error:
            return base_progress * 0.9
        elif completed:
            return base_progress * 1.0
        else:
    ...
    def _finalize_debate_results(self, context_persona_turn_results: Optional[Dict[str, Any]], 
        ...
        elif context_persona_turn_results:

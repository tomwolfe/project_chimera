...
def llm_provider():
    ...
    with patch('src.llm_provider.GeminiProvider.generate') as mock_generate:
        mock_generate.return_value = ("This is a mocked LLM response.", 100, 50) # (text, input_tokens, output_tokens)
        ...
        provider = LLMProvider(api_key="dummy-api-key", model_name="gemini-2.5-flash-lite")
        yield provider
...
def test_llm_provider_generate_content_success(llm_provider):
    ...
    response_text, _, _ = llm_provider.generate(prompt, system_prompt="You are a helpful assistant.", temperature=0.4, max_tokens=100)
...
def test_llm_provider_generate_content_error_handling(llm_provider):
    ...
    with patch('src.llm_provider.GeminiProvider.generate', side_effect=Exception("API Error")):
        with pytest.raises(Exception, match="API Error"):
            llm_provider.generate("This prompt should cause an error.", system_prompt="You are a helpful assistant.", temperature=0.4, max_tokens=100)

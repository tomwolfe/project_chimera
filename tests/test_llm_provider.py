...
def test_gemini_provider_generate_api_error_retry(gemini_provider, mock_genai_client):
    ...
    mock_genai_client.models.generate_content.side_effect = [
        ...
        MagicMock(
            candidates=[MagicMock(content=MagicMock(parts=[MagicMock(text="Retry success")]))]
...
def test_gemini_provider_generate_api_error_max_retries_exceeded(
    gemini_provider, mock_genai_client
):
    """Test API error exceeding max retries."""
    mock_genai_client.models.generate_content.side_effect = APIError(
        "Persistent error", code=500
    )
    ...
    with pytest.raises(LLMProviderError):  # Expect LLMProviderError (or GeminiAPIError)
        gemini_provider.generate(
            prompt="Test max retry", system_prompt="", temperature=0.5, max_tokens=100
        )
...
def test_gemini_provider_circuit_breaker_open(gemini_provider, mock_genai_client):
    ...
    gemini_provider.generate.circuit_breaker.failures = (
        gemini_provider.generate.circuit_breaker.failure_threshold - 1
    )
    mock_genai_client.models.generate_content.side_effect = APIError(
        "Failure to open CB", code=500
    )

    with pytest.raises(APIError):
        gemini_provider.generate(
            prompt="Fail once", system_prompt="", temperature=0.5, max_tokens=100
        )
    ...
    with pytest.raises(CircuitBreakerError, match="Service unavailable: Circuit breaker is open"):
        gemini_provider.generate(
            prompt="Blocked by CB", system_prompt="", temperature=0.5, max_tokens=100
        )
...
def test_gemini_provider_context_window_exceeded_error(gemini_provider, mock_genai_client):
    ...
    mock_genai_client.models.generate_content.side_effect = APIError(
        "Prompt too large", code=400
    )

    with pytest.raises(LLMProviderError, match="LLM context window exceeded"):
        gemini_provider.generate(
            prompt="Very long prompt", system_prompt="", temperature=0.5, max_tokens=100
        )

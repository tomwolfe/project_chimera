...
class TestGeminiProvider(unittest.TestCase):
    ...
    def test_generate_with_valid_json(self):
        ...
        mock_response.candidates = [
            MagicMock(
                content=MagicMock(parts=[MagicMock(text='{"test_field": "value"}')])
            )
        ]
    ...
    def test_clean_generated_text(self):
        ...
        cleaned_surrounding = self.provider.output_parser._clean_llm_output(
            text_surrounding
        )
        ...
        cleaned_malformed = self.provider.output_parser._clean_llm_output(
            text_malformed
        )
        self.assertEqual(cleaned_malformed, text_malformed)

        text_incomplete = '{"test": "value"'
        cleaned_incomplete = self.provider.output_parser._clean_llm_output(
            text_incomplete
        )

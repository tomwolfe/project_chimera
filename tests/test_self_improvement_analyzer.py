...
try:
    ...
    from src.models import SelfImprovementAnalysisOutputV1, SuggestionItem # NEW: Import specific models
...
def mock_metrics_collector():
    ...
    collector.collect_all_metrics.return_value = {
        ...
        "maintainability": {"test_coverage_summary": {"overall_coverage_percentage": 100.0}},
...
def mock_llm_provider():
    ...
    provider.generate.return_value = ('{"ANALYSIS_SUMMARY": "Mock analysis summary.", "IMPACTFUL_SUGGESTIONS": []}', 100, 50, False)
    return provider
...
def mock_persona_manager():
    ...
    pm.get_adjusted_persona_config.return_value = MagicMock(system_prompt="Analyst", temperature=0.1, max_tokens=8192)
    return pm
...
def test_self_improvement_analyzer_initialization(
    mock_metrics_collector, mock_llm_provider, mock_persona_manager, mock_content_validator, mock_tokenizer
):
...
def test_analyze_self_improvement_plan_historical_low_success_rate(
    ...
    assert any("Overall self-improvement success rate is low" in s["PROBLEM"] for s in suggestions)
    assert any("Review the `Self_Improvement_Analyst`'s system prompt" in s["PROPOSED_SOLUTION"] for s in suggestions)
...
def test_analyze_self_improvement_plan_historical_common_failure_mode(
    ...
    mock_metrics_collector.analyze_historical_effectiveness.return_value = {
        ...
        "success_rate": 0.8,
        "top_performing_areas": [],
        "common_failure_modes": {"schema_validation_failures_count": 5}, # Common failure
        "historical_total_suggestions_processed": 10,
    ...
    suggestions = analyzer.analyze()
    assert any("Frequent schema validation failures" in s["PROBLEM"] for s in suggestions)
    assert any("Refine JSON output instructions in persona prompts" in s["PROPOSED_SOLUTION"] for s in suggestions)
...
def test_analyze_self_improvement_plan_high_token_consumption(
    ...
    mock_metrics_collector.collect_all_metrics.return_value = {
        ...
        "performance_efficiency": {
            "token_usage_stats": {"persona_token_usage": {"TestPersona": 3000, "OtherPersona": 500}},
            "debate_efficiency_summary": {"persona_token_breakdown": {"TestPersona": 3000, "OtherPersona": 500}},
        },
        "robustness": {"schema_validation_failures_count": 0},
        "maintainability": {"test_coverage_summary": {"overall_coverage_percentage": 100.0}},
        "reasoning_quality": {"content_misalignment_warnings": 0},
        "historical_analysis": {
            "total_attempts": 0, "success_rate": 0.0, "top_performing_areas": [], "common_failure_modes": {},
            "historical_total_suggestions_processed": 0, "historical_successful_suggestions": 0, "historical_schema_validation_failures": {},
        },
    }
    mock_llm_provider.generate.return_value = (
        json.dumps({
    ...
    suggestions = analyzer.analyze()
    assert any("High token consumption by personas" in s["PROBLEM"] for s in suggestions)
    assert any("Optimize prompts for high-token personas" in s["PROPOSED_SOLUTION"] for s in suggestions)
...
def mock_persona_manager():
    ...
    pm.all_personas = {
        ...
        "Test_Engineer": PersonaConfig(name="Test_Engineer", system_prompt="You are a test engineer.", temperature=0.5, max_tokens=1024), # Added Test_Engineer
        "Impartial_Arbitrator": PersonaConfig(name="Impartial_Arbitrator", system_prompt="You are an impartial arbitrator.", temperature=0.5, max_tokens=1024), # Added Impartial_Arbitrator
    }
    pm.persona_sets = {
...
def test_determine_persona_sequence_self_analysis(persona_router_instance, mock_persona_manager):
    ...
    assert sequence.index("SelfImprovementAnalyst") < sequence.index("Impartial_Arbitrator")
...
def test_determine_persona_sequence_with_context_analysis(persona_router_instance):
    ...
    assert "Code_Architect" not in sequence # Should not be added without architectural concerns, unless it's a default for SE
...
def test_should_include_test_engineer_self_improvement_domain(persona_router_instance):
    ...
    context_analysis_results = {}
    domain = "self-improvement"
    assert persona_router_instance._should_include_test_engineer(prompt.lower(), context_analysis_results, domain) is True
...
def test_should_include_test_engineer_with_test_files_in_context(persona_router_instance):
...
def test_should_include_test_engineer_no_relevant_triggers(persona_router_instance):
    ...
    domain = "Software Engineering"
    assert persona_router_instance._should_include_test_engineer(prompt.lower(), context_analysis_results, domain) is False
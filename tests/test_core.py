...
class MockPersonaManager:
    ...
    def all_personas(self):
        return {
            "MockPersona1": MagicMock(
                name="MockPersona1",
                system_prompt="Mock prompt 1",
                temperature=0.7,
                max_tokens=100,
            ),
            "MockPersona2": MagicMock(
                name="MockPersona2",
                system_prompt="Mock prompt 2",
                temperature=0.3,
                max_tokens=100,
            ),
        }
...
def chimera_instance():
    ...
    with (
        patch("src.core.GeminiProvider", return_value=MockLLMProvider()),
        ...
        patch(
            "src.utils.output_parser.LLMOutputParser", return_value=MockOutputParser()
        ),
    ):
...
def test_chimera_run_debate_simple_flow(chimera_instance):
    ...
    mock_llm_provider.generate.return_value = (
        ...
        100,
        50,
    )  # Simulate return tuple

    mock_persona_manager = chimera_instance.persona_manager
    mock_persona_manager.get_persona_sequence_for_framework.return_value = [
        "MockPersona1",
        "MockPersona2",
    ]

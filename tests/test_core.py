...
def mock_llm_provider():
    ...
    provider.generate.return_value = ('{"general_output": "Mock response"}', 100, 50, False)
    return provider
...
def mock_persona_manager(mock_token_tracker, mock_settings):
    ...
    pm.all_personas = {
        ...
        "Conflict_Resolution_Manager": PersonaConfig(name="Conflict_Resolution_Manager", system_prompt="Mediator", temperature=0.1, max_tokens=4096),
    ...
    pm.persona_sets = {
        "General": ["Visionary_Generator", "Skeptical_Generator", "Constructive_Critic", "Devils_Advocate", "Conflict_Resolution_Manager", "Impartial_Arbitrator"],
    }
    ...
    pm.persona_router.determine_persona_sequence.return_value = ["Constructive_Critic", "Devils_Advocate", "Conflict_Resolution_Manager", "Impartial_Arbitrator"]
...
def mock_output_parser():
    ...
    parser.parse_and_validate.side_effect = [
        {"CRITIQUE_SUMMARY": "Mock critique", "CRITIQUE_POINTS": [], "SUGGESTIONS": [], "malformed_blocks": []},
        {"conflict_type": "NO_CONFLICT", "summary": "No conflict reported", "involved_personas": [], "conflicting_outputs_snippet": "", "proposed_resolution_paths": [], "conflict_found": False, "malformed_blocks": []},
...
def socratic_debate_instance(
    ...
    with patch('core.LLMOutputParser', return_value=mock_output_parser), \
...
def test_socratic_debate_run_flow_simple(socratic_debate_instance, mock_llm_provider, mock_output_parser):
    ...
    mock_llm_provider.generate.side_effect = [
        # Constructive_Critic
        ('{"CRITIQUE_SUMMARY": "Critique output", "CRITIQUE_POINTS": [], "SUGGESTIONS": []}', 100, 50, False),
        # Devils_Advocate (no conflict)
        ('{"conflict_type": "NO_CONFLICT", "summary": "No conflict reported", "involved_personas": [], "conflicting_outputs_snippet": "", "proposed_resolution_paths": [], "conflict_found": false}', 100, 50, False),
    ...
    mock_output_parser.parse_and_validate.side_effect = [
        {"CRITIQUE_SUMMARY": "Critique output", "CRITIQUE_POINTS": [], "SUGGESTIONS": [], "malformed_blocks": []},
        {"conflict_type": "NO_CONFLICT", "summary": "No conflict reported", "involved_personas": [], "conflicting_outputs_snippet": "", "proposed_resolution_paths": [], "conflict_found": False, "malformed_blocks": []},
    ...
    assert "Constructive_Critic" in [t['persona'] for t in intermediate_steps['Debate_History']]
    assert "Devils_Advocate" in [t['persona'] for t in intermediate_steps['Debate_History']]
    assert "Impartial_Arbitrator" in [t['persona'] for t in intermediate_steps['Debate_History']]
    assert "Conflict_Resolution_Manager" not in [t['persona'] for t in intermediate_steps['Debate_History']] # Should not be called if no conflict

def test_socratic_debate_conflict_resolution_flow(socratic_debate_instance, mock_llm_provider, mock_output_parser, mock_conflict_manager):
    ...
    mock_llm_provider.generate.side_effect = [
        ...
        ('{"CRITIQUE_SUMMARY": "Problematic critique", "malformed_blocks": [{"type": "SCHEMA_VALIDATION_ERROR"}]}', 100, 50, False),
        # Devils_Advocate (reports conflict)
        ('{"conflict_type": "LOGICAL_INCONSISTENCY", "summary": "Conflict detected", "involved_personas": ["Constructive_Critic"], "conflicting_outputs_snippet": "...", "proposed_resolution_paths": ["Mediate"], "conflict_found": true}', 100, 50, False),
        # Impartial_Arbitrator (receives resolved output from mock_conflict_manager)
        ('{"general_output": "Final answer after conflict resolution"}', 100, 50, False),
    ]
    # Mock parser to return problematic for critic, then conflict report, then final answer
    mock_output_parser.parse_and_validate.side_effect = [
        {"CRITIQUE_SUMMARY": "Problematic critique", "CRITIQUE_POINTS": [], "SUGGESTIONS": [], "malformed_blocks": [{"type": "SCHEMA_VALIDATION_ERROR"}]},
        {"conflict_type": "LOGICAL_INCONSISTENCY", "summary": "Conflict detected", "involved_personas": ["Constructive_Critic"], "conflicting_outputs_snippet": "...", "proposed_resolution_paths": ["Mediate"], "conflict_found": True, "malformed_blocks": []},
        {"general_output": "Final answer after conflict resolution", "malformed_blocks": []},
    ]

    final_answer, intermediate_steps = socratic_debate_instance.run_debate()

    assert final_answer is not None
    assert "Final answer after conflict resolution" in final_answer.get("general_output", "")
    ...
    assert "Conflict_Resolution_Manager" in [t['persona'] for t in intermediate_steps['Debate_History']] # Should be called
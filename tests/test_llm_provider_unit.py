...
def test_llm_provider_generate_malformed_response_attribute_error():
    ...
    mock_response_missing_parts.candidates[0].content.parts = [] # This will cause an IndexError
...
def test_llm_provider_generate_with_schema_validation_success():
    ...
    mock_response.candidates[0].content = MagicMock()
    mock_response.candidates[0].content.parts = [MagicMock()]
    mock_response.candidates[0].content.parts[0].text = '{"general_output": "Valid output"}'
    mock_client.models.generate_content.return_value = mock_response
...
def test_llm_provider_generate_with_schema_validation_failure():
    ...
    mock_response.candidates[0].content = MagicMock()
    mock_response.candidates[0].content.parts = [MagicMock()]
    mock_response.candidates[0].content.parts[0].text = '{"invalid_field": "Malformed output"}' # Does not match GeneralOutput
    mock_client.models.generate_content.return_value = mock_response
    ...
    with patch('src.llm_provider.genai.Client', return_value=mock_client):
        ...
        with pytest.raises(SchemaValidationError, match="LLM output failed schema validation"):
            provider.generate(

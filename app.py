...
def main():
    ...
    with col1:
        ...
        with st.expander("⚙️ Custom Framework Management", expanded=False):
            ...
            with tabs_framework[0]:
                ...
                if (
                    st.button(
                        "Save Current Framework", on_click=update_activity_timestamp
                    )
            ...
            with tabs_framework[1]:
                ...
                if (
                    st.button(
                        "Load Selected Framework", on_click=update_activity_timestamp
                    )
            ...
            with tabs_framework[2]:
                ...
                st.info(
                    "Export the currently selected framework to a file for sharing."
                )
        ...
        for p_name in sorted_persona_names:
            ...
            with st.expander(f"**{persona.name.replace('_', ' ')}**", expanded=False):
                ...
                if st.button(
                    ...
                    update_activity_timestamp()
                    if st.session_state.persona_manager.reset_persona_to_default(
                        p_name
                    ):
                        st.toast(
                            f"Persona '{p_name.replace('_', ' ')}' reset to default."
                        )
                        st.session_state.persona_changes_detected = False
                        st.rerun()
                    else:
                        st.error(
                            "Could not reset all personas for the current framework."
                        )
    ...
    def _run_socratic_debate_process():
        ...
        if is_self_analysis_prompt_detected:
            ...
            try:
                ...
                st.session_state.context_analyzer._last_raw_file_contents_hash = hash(
                    frozenset(
                        st.session_state.context_analyzer.raw_file_contents.items()
                    )
                )
        ...
        if sanitized_prompt != user_prompt:
            st.warning(
                "User prompt was sanitized to mitigate potential injection risks."
            )
        ...
        with st.status("Socratic Debate in Progress", expanded=True) as status:
            ...
            def capture_rich_output_and_get_console():
                ...
                console_instance = Console(
                    file=buffer, force_terminal=True, soft_wrap=True
                )
            ...
            with capture_rich_output_and_get_console() as (
                ...
                try:
                    ...
                    if (
                        ...
                        if (
                            ...
                            try:
                                ...
                                st.error(
                                    f"❌ Error re-computing context embeddings: {e}"
                                )
                                logger.error(
                    ...
                    if (
                        ...
                        logger.debug(
                            "Captured file_analysis_cache from debate instance."
                        )
                    else:
                        st.session_state.file_analysis_cache = None
                    ...
                    status.update(
                        label="Socratic Debate Complete!",
                        state="complete",
                        expanded=False,
                    )
                    ...
                    log_event(
                        f"Socratic Debate failed with error: {e}",
                        level="error",
                        data={"error": str(e)},
                    )
                finally:
    ...
    if st.session_state.debate_ran:
        ...
        if actual_debate_domain == "Software Engineering":
            ...
            if (
                ...
                all_malformed_blocks.extend(
                    validation_results_by_file["malformed_blocks"]
                )
            ...
            if (
                not parsed_llm_output_dict.get("CODE_CHANGES")
                and not all_malformed_blocks
            ):
                    ...
                    if change.get("ACTION") in [
                        ...
                        if change.get("ACTION") == "MODIFY":
                            ...
                            if original_content:
                                ...
                                st.code(
                                    change.get("FULL_CONTENT", ""), language="python"
                                )
                        else:
                            st.write("**Content:**")
                            display_content = (
        ...
        with st.expander("Show Intermediate Steps & Process Log"):
            ...
            st.code(
                strip_ansi_codes(st.session_state.process_log_output_text),
                language="text",
            )


if __name__ == "__main__":
    # Set the global exception hook to use our custom handler
    sys.excepthook = error_handling_handle_exception
